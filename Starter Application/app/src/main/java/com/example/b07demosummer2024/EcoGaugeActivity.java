...
public class EcoGaugeActivity extends AppCompatActivity {
    ...
    BarChart byCountry;
    LineChart history;
    HashMap<String, Double> countryConstantsMonthly = new HashMap<>();
    HashMap<String, Double> countryConstantsWeekly = new HashMap<>();
    HashMap<String, Double> countryConstantsDaily = new HashMap<>();
    XAxis xAxisLine;
    ArrayList<HashMap<String, Float[]>[]> weeks;
    ArrayList<HashMap<String, Float[]>[]> months;
...
//      Default setting the gauge to display annual
        ...
        String userID = "1dJoXS8M5dZqRc1XyDhOPmq01bH2";
...
//                Changes the scale of carbon by country depending on if it's weeks,months, years
                if (counter == 3) {
                    ...
                    updateEntries.add(new BarEntry(1f, Objects.requireNonNull(countryConstants.get(selectedCountry)).floatValue()/365));
                }
                if (counter == 2) {
                    updateEntries.add(new BarEntry(0f, totalCarbon.floatValue()));
                    updateEntries.add(new BarEntry(1f, Objects.requireNonNull(countryConstants.get(selectedCountry)).floatValue()/12));
                }
                if (counter == 1) {
                    updateEntries.add(new BarEntry(0f, totalCarbon.floatValue()));
                    updateEntries.add(new BarEntry(1f, Objects.requireNonNull(countryConstants.get(selectedCountry)).floatValue()/52));
                }
//                sets the values
...
//        Bar Chart
            ...
            updateEntries.add(new BarEntry(0f, totalCarbon.floatValue()));
            System.out.println(countryConstantsWeekly.keySet());
            updateEntries.add(new BarEntry(1f, Objects.requireNonNull(countryConstants.get(selectedCountry)).floatValue()/52));
...
//            Line chart
            HashMap<String, Float[]> lineChartData= last30final;
...
//          This is ok because there's only one key in each hashmap
    ...
    private void setVarsDaily(){
        ...
        if (lastIndex != -1) {
            for (Object day : currWeek[lastIndex].keySet()) {
                Float[] latestDay = currWeek[lastIndex].get(day);
    ...
    private void getWeeksTimeline(String userID) {
        ...
        DatabaseReference ref = FirebaseDatabase.getInstance("https://b07finalproject-4e3be-default-rtdb.firebaseio.com/")
                .getReference("users/"+userID+"/dailylogs/");
        ...
        ref.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                HashMap<String, Float[]>[] currWeek = new HashMap[7];
                int dayIndex = 0;
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (String currDateStr : dateKeys) {
                    ...
                    if (emissions != null && transportation != null && consumption != null && housing != null && food != null) {
                        ...
                        dailyCarbonInfo = new Float[]{0F, 0F, 0F, 0F, 0F};
                    }
                    dayEntry.put(currDateStr, dailyCarbonInfo);
    ...
    HashMap<String, Float[]> last30final = new HashMap<>();
